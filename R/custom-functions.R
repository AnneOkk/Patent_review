...
make_composite <- function(df) {
  data_comp <- dplyr::select_if(df, is.numeric) %>% remove_all_labels(.) %>% dplyr::select(matches("_\\d+"))
  data <- df %>% dplyr::select_if(., is.numeric) 
  data <- data %>% dplyr::select(str_c(setdiff(colnames(data), colnames(data_comp))))
  data_comp <- data_comp %>% split.default(sub("_.*", "", names(data_comp)))
  comp <- purrr::map(data_comp, ~ rowMeans(.x, na.rm=T))
